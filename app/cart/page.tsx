        ...
        <div className="container mx-auto px-4">
          {items.length > 0 ? (
            <div className="grid lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
              ...
              <div className="lg:col-span-2 space-y-3 sm:space-y-4">
                {items.map((item, index) => (
                  ...
                  >
                    <div className="relative w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 rounded-lg sm:rounded-xl overflow-hidden bg-[#FBF6EC] flex-shrink-0">
                      <img
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm sm:text-base md:text-xl font-semibold text-[#1A1A1A] mb-0.5 sm:mb-1 md:mb-2 truncate">{item.name}</h3>
                      {(pricing.linePricingById[item.id]?.discount || 0) > 0 && (
                        <span className="inline-flex items-center gap-1 bg-gradient-to-r from-[#e53e3e] to-[#8E6D31] text-white px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-bold mb-1.5">
                          🔥 Rs {pricing.linePricingById[item.id].discount.toLocaleString()} OFF Applied
                        </span>
                      )}
                      <p className="text-base sm:text-lg md:text-2xl font-bold text-[#C5A059] mb-2 sm:mb-3 md:mb-4">
                        Rs. {item.price.toLocaleString()}
                      </p>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-1.5 sm:space-x-2 md:space-x-3 bg-[#FBF6EC] rounded-full px-2 sm:px-3 md:px-4 py-1 sm:py-1.5 md:py-2">
                          <button
                          ...
                          <span className="font-semibold text-[#1A1A1A] w-5 sm:w-6 md:w-8 text-center text-xs sm:text-sm md:text-base">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => updateQuantity(item.id, 1)}
                      ...
                      <p className="text-xs sm:text-sm md:text-lg font-semibold text-[#1A1A1A] mt-2 sm:mt-3 md:mt-4">
              ...
              >
                <div className="bg-[#FBF6EC] rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 sticky top-24">
                  {/* 1 KG Promo Banner */}
                  <div className="bg-gradient-to-r from-[#e53e3e] to-[#8E6D31] text-white rounded-lg p-2.5 mb-4 text-center">
                  ...
                  <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-[#1A1A1A] mb-4 sm:mb-5 md:mb-6">Order Summary</h2>
                  <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-5 md:mb-6">
                    ...
                    <div className="border-t border-gray-300 pt-3 sm:pt-4">
                      <div className="flex justify-between text-base sm:text-lg md:text-xl font-bold text-[#1A1A1A]">
                        <span>Total</span>
                  ...
                  <Link href="/checkout">
                    <button
                      className="w-full bg-[#C5A059] hover:bg-[#1A1A1A] text-white px-4 sm:px-6 md:px-8 py-3 sm:py-3.5 md:py-4 rounded-full font-semibold transition-all duration-200 active:scale-[0.98] flex items-center justify-center gap-2 text-sm sm:text-base"
                    >
                  ...
                  <Link href="/shop">
                    <button
                      className="w-full mt-3 sm:mt-4 bg-white hover:bg-[#F5EDD8] text-[#1A1A1A] px-4 sm:px-6 md:px-8 py-3 sm:py-3.5 md:py-4 rounded-full font-semibold transition-all duration-200 active:scale-[0.98] flex items-center justify-center gap-2 border-2 border-[#1A1A1A] text-sm sm:text-base"
                    >
            ...
            >
              ...
              <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#1A1A1A] mb-2 sm:mb-4">Your cart is empty</h2>
              <p className="text-sm sm:text-base text-[#6B7280] mb-6 sm:mb-8">Start adding items to your cart!</p>
              <Link href="/shop">
                <button
                  className="bg-[#C5A059] hover:bg-[#1A1A1A] text-white px-6 sm:px-8 py-3 sm:py-4 rounded-full font-semibold transition-all duration-200 active:scale-[0.98] text-sm sm:text-base"
                >
                  Continue Shopping
                </button>
              </Link>
