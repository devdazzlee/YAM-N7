        ...
        <div className="luxury-container">
          {items.length > 0 ? (
            <div className="grid lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
              <div className="lg:col-span-2 space-y-3 sm:space-y-4">
                {items.map((item, index) => (
                  <motion.div
                    ...
                    className="bg-background border border-foreground/10 p-3 sm:p-4 md:p-6 flex gap-3 sm:gap-4 md:gap-6"
                  >
                    <div className="relative w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 overflow-hidden bg-surface flex-shrink-0 border border-foreground/10">
                      <img
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm sm:text-base md:text-xl font-semibold text-foreground mb-0.5 sm:mb-1 md:mb-2 truncate">
                        {item.name}
                      </h3>
                      {(pricing.linePricingById[item.id]?.discount || 0) > 0 && (
                        <span className="inline-flex items-center gap-1 bg-primary text-primary-foreground px-2 py-0.5 text-[9px] sm:text-[10px] font-bold mb-1.5">
                          Rs {pricing.linePricingById[item.id].discount.toLocaleString()} OFF Applied
                        </span>
                      )}
                      <p className="text-base sm:text-lg md:text-2xl font-bold text-foreground mb-2 sm:mb-3 md:mb-4">
                        Rs. {item.price.toLocaleString()}
                      </p>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-1.5 sm:space-x-2 md:space-x-3 border border-foreground/10 bg-surface px-2 sm:px-3 md:px-4 py-1 sm:py-1.5 md:py-2">
                          <button
                            onClick={() => updateQuantity(item.id, -1)}
                            className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 flex items-center justify-center hover:bg-background transition-colors"
                          >
                        ...
                        >
                          <Trash2 className="w-4 h-4 sm:w-5 md:w-5 h-5" />
                        </button>
                      </div>
              ...
              >
                <div className="bg-background border border-foreground/10 p-4 sm:p-5 md:p-6 sticky top-24">
                  <div className="bg-primary text-primary-foreground p-2.5 mb-4 text-center">
                    <p className="text-xs sm:text-sm font-bold">1 KG = Rs {KG_DISCOUNT.amount} OFF</p>
                    <p className="text-[10px] sm:text-[11px] text-primary-foreground/80">On all products</p>
                  </div>
                  <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-foreground mb-4 sm:mb-5 md:mb-6">
                    Order Summary
                  </h2>
                  <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-5 md:mb-6">
                    ...
                    {kgDiscountTotal > 0 && (
                      <div className="flex justify-between text-primary text-sm sm:text-base font-semibold">
                  ...
                  <Link href="/checkout">
                    <button className="w-full luxury-btn-primary flex items-center justify-center gap-2">
                      <span>Proceed to Checkout</span>
                      <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5" />
                    </button>
                  </Link>
                  <Link href="/shop">
                    <button className="w-full mt-3 sm:mt-4 luxury-btn-outline flex items-center justify-center gap-2">
            ...
            >
              <div className="max-w-md mx-auto border border-foreground/10 bg-surface px-6 py-12 sm:px-10">
                <ShoppingBag className="w-12 h-12 text-primary mx-auto mb-4" strokeWidth={1.5} />
                <h2 className="text-xl sm:text-2xl font-bold text-foreground mb-2">Your cart is empty</h2>
                <p className="text-sm text-muted mb-6">Start adding items to your cart.</p>
