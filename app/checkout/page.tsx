...
export default function CheckoutPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Page Header */}
      <section className="bg-gradient-to-r from-[#1A1A1A] to-[#C5A059] text-white py-8 sm:py-12 md:py-16">
      ...
      <section className="py-6 sm:py-8 md:py-12 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="lg:hidden mb-4">
            <details className="bg-[#FBF6EC] rounded-xl overflow-hidden">
              <summary className="flex items-center justify-between p-3.5 sm:p-4 cursor-pointer select-none">
                ...
                <span className="font-bold text-[#1A1A1A] text-sm">Rs. {total.toLocaleString()}</span>
              </summary>
              <div className="px-3.5 sm:px-4 pb-3.5 sm:pb-4 space-y-3">
                {cartItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-2.5 pb-2.5 border-b border-gray-200/60 last:border-0 last:pb-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-[#1A1A1A] text-xs sm:text-sm truncate">{item.name}</p>
                      <p className="text-[10px] sm:text-xs text-[#6B7280]">Qty: {item.quantity}</p>
                      {(pricing.linePricingById[item.id]?.discount || 0) > 0 && (
                        <span className="inline-block bg-gradient-to-r from-[#e53e3e] to-[#8E6D31] text-white px-1.5 py-[1px] rounded-full text-[8px] font-bold mt-0.5">
                          🔥 Rs {pricing.linePricingById[item.id].discount.toLocaleString()} OFF
                        </span>
                      )}
                    </div>
                    <p className="font-bold text-[#C5A059] text-xs sm:text-sm whitespace-nowrap">
                ...
                <div className="pt-2 space-y-1.5 text-xs sm:text-sm">
                  ...
                  <div className="border-t border-gray-300 pt-2 mt-1">
                    <div className="flex justify-between font-bold text-[#1A1A1A] text-sm sm:text-base">
                      <span>Total</span>
          ...
          <div className="grid lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
            ...
            <div className="lg:col-span-2">
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                {step === 1 && (
                  ...
                  >
                    <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-[#1A1A1A] mb-2 sm:mb-4 md:mb-6">Shipping Information</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
                      <div>
                        <label className="block text-[#1A1A1A] font-medium mb-1.5 sm:mb-2 text-sm sm:text-base">
                          First Name *
                        </label>
                        <input
                          type="text"
                          ...
                          className="w-full px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg sm:rounded-xl border border-gray-300 focus:border-[#C5A059] focus:ring-2 focus:ring-[#C5A059]/20 outline-none text-sm sm:text-base"
                        />
                      </div>
                      <div>
                        <label className="block text-[#1A1A1A] font-medium mb-1.5 sm:mb-2 text-sm sm:text-base">
                    ...
                    <div>
                      <label className="block text-[#1A1A1A] font-medium mb-1.5 sm:mb-2 text-sm sm:text-base">Email *</label>
                      <input
                        type="email"
                        required
                    ...
                    <div>
                      <label className="block text-[#1A1A1A] font-medium mb-1.5 sm:mb-2 text-sm sm:text-base">Phone Number *</label>
                      <input
                        type="tel"
                        required
                    ...
                    <div>
                      <label className="block text-[#1A1A1A] font-medium mb-1.5 sm:mb-2 text-sm sm:text-base">Delivery Address *</label>
                      <textarea
                        required
                    ...
                    <div>
                      <label className="block text-[#1A1A1A] font-medium mb-1.5 sm:mb-2 text-sm sm:text-base">City *</label>
                      <input
                        type="text"
                        required
                        value={formData.city}
                    ...
                    <div>
                      <label className="block text-[#1A1A1A] font-medium mb-1.5 sm:mb-2 text-sm sm:text-base">Order Notes (Optional)</label>
                      <textarea
                ...
                {step === 2 && (
                  <motion.div
                    ...
                    className="bg-white border border-gray-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-5 md:space-y-6"
                  >
                    <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-[#1A1A1A] mb-2 sm:mb-4 md:mb-6">Payment Method</h2>
                    <div className="space-y-3 sm:space-y-4">
                      <div className="flex items-start gap-3 sm:space-x-4 p-4 sm:p-5 md:p-6 border-2 border-[#C5A059] rounded-xl bg-[#F5EDD8]/30">
                        <Truck className="w-5 h-5 sm:w-6 sm:h-6 text-[#C5A059] mt-0.5 flex-shrink-0" />
                        <div className="flex-1">
                          <p className="font-bold text-[#1A1A1A] text-sm sm:text-base md:text-lg">Cash on Delivery</p>
                          <p className="text-xs sm:text-sm text-[#6B7280] mt-1">
                            Pay with cash when your order is delivered. Our delivery person will collect the payment.
                          </p>
                          <div className="mt-2 sm:mt-3 flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-[#C5A059]">
                    ...
                    <div className="mt-4 sm:mt-6 md:mt-8 pt-4 sm:pt-6 border-t border-gray-200">
                      <h3 className="text-base sm:text-lg font-bold text-[#1A1A1A] mb-3 sm:mb-4">Order Review</h3>
                ...
                <div className="flex gap-2.5 sm:gap-3 md:space-x-4">
                  {step > 1 && (
                    <button
                      ...
                      disabled={loading}
                      className="px-4 sm:px-6 md:px-8 py-3 sm:py-3.5 md:py-4 bg-gray-200 hover:bg-gray-300 text-[#1A1A1A] rounded-full font-semibold transition-colors disabled:opacity-50 text-sm sm:text-base"
                    >
                      Back
                    </button>
                  )}
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 bg-[#C5A059] hover:bg-[#1A1A1A] text-white px-4 sm:px-6 md:px-8 py-3 sm:py-3.5 md:py-4 rounded-full font-semibold transition-all duration-200 active:scale-[0.98] flex items-center justify-center gap-1.5 sm:gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                  >
                    {loading ? (
                      <>
            ...
            >
              <div className="bg-[#FBF6EC] rounded-2xl p-5 md:p-6 sticky top-24">
                <h2 className="text-lg md:text-xl font-bold text-[#1A1A1A] mb-4 md:mb-6">Order Summary</h2>
                <div className="space-y-3 md:space-y-4 mb-4 md:mb-6">
                  {cartItems.map((item) => (
                    <div key={item.id} className="flex items-center gap-3 pb-3 border-b border-gray-200">
                      ...
                      />
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-[#1A1A1A] text-xs md:text-sm truncate">{item.name}</p>
                        <p className="text-[10px] md:text-xs text-[#6B7280]">Qty: {item.quantity}</p>
                        {(pricing.linePricingById[item.id]?.discount || 0) > 0 && (
                          <span className="inline-block bg-gradient-to-r from-[#e53e3e] to-[#8E6D31] text-white px-1.5 py-[1px] rounded-full text-[8px] md:text-[9px] font-bold mt-0.5">
                            🔥 Rs {pricing.linePricingById[item.id].discount.toLocaleString()} OFF
                          </span>
                        )}
                      </div>
                      <p className="font-bold text-[#C5A059] text-sm whitespace-nowrap">
                        Rs. {(pricing.linePricingById[item.id]?.total || 0).toLocaleString()}
                      </p>
                    </div>
                  ))}
